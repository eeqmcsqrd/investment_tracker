...
def svg_to_base64(file_path):
    try:
        ...
        print(f"Successfully loaded SVG from {file_path}")
...
def get_icon(icon_path, emoji_fallback, width=24, height=24, style=""):
    ...
    if base_icon:
        # Make sure we're using the correct HTML syntax for images
...
<style>
    ...
    /* Improved styling for SVG icons */
    img[width="24"], img[width="32"] {
    ...
    .streamlit-expanderHeader img {
        margin-right: 8px;
    }
    /* Icon color styling for status messages */
    .stAlert img {
...
with st.sidebar:
    ...
    if entry_tab == "Single Entry":
        ...
        if st.button(f"{ICONS['add_entry']} Add Entry", key="add_single", use_container_width=True):
...
with tab1:
    ...
    if not latest_df.empty:
        ...
        try:
            ...
            if len(previous_dates) > 0:
                ...
                for _, row in latest_values.iterrows():
                    ...
                    if not prev_entries.empty:
                        ...
                        changes_data.append({
                            ...
                            "_change_pct": 0,   # For conditional formatting
                            # Add these numeric columns for sorting
                            "Current Value Num": current_value_usd,
                            "Change Num": 0,
                            "Change Pct Num": 0
                ...
                if changes_data:
                    ...
                    with filter_col3:
                        search_inv = st.text_input(f"{ICONS['search']} Search", key="changes_search")
                    ...
                    if sort_by == "Change (Abs)":
                        ...
                        changes_df = changes_df.sort_values("Current Value Num", ascending=False)
                    ...
                    st.info(f"{ICONS['info']} No investment changes to display.")
            else:
                st.info(f"{ICONS['info']} No historical data available for comparison.")
        except Exception as e:
            st.error(f"{ICONS['error']} Error calculating investment changes: {str(e)}")
        st.markdown("---")
        ...
        with chart_col2:
            ...
            st.plotly_chart(fig, use_container_width=True, config={'displayModeBar': False})
        # Bottom charts row with enhanced category visualization
    ...
    if not df.empty:
        ...
        if date_filter_type == "Range":
            ...
            if date_preset == "Custom":
                ...
                st.info(f"{ICONS['info']} Date range: {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}")
        else:
            ...
            with data_cols[1]:
                if specific_date_type == "Specific Date":
                    ...
                    st.info(f"{ICONS['info']} Using latest date: {latest_date.strftime('%Y-%m-%d')}")
        ...
        if export_format == "CSV":
            ...
            st.download_button(
                label=f"{ICONS['export']} Download Data",
                data=csv,
            ...
            st.download_button(
                label=f"{ICONS['export']} Download Excel",
                data=excel_data,
            ...
            st.download_button(
                label=f"{ICONS['export']} Download JSON",
                data=json_str,
...
with tab4:
    ...
    with settings_tab1:
        ...
        if not df.empty:
            ...
            if backup_format == "CSV":
                ...
                st.download_button(
                    label=f"{ICONS['save']} Export All Data",
                    data=csv,
                ...
                if st.button(f"{ICONS['import']} Confirm Import", key="confirm_import", use_container_width=True):
                    with st.spinner("Processing import..."):
                        ...
                        if import_action == "Replace all data":
                            save_data(import_df)
                            st.success(f"{ICONS['success']} Data replaced successfully!")
                            st.session_state.show_success = True
                            st.rerun()
                            ...
                            st.success(f"{ICONS['success']} Data appended successfully!")
                            st.session_state.show_success = True
                            st.rerun()
                            ...
                            save_data(result_df)
                            st.success(f"{ICONS['success']} Data updated successfully!")
                            st.session_state.show_success = True
                            st.rerun()
            except Exception as e:
    ...
    with settings_tab2:
        ...
        st.info(
            f"{ICONS['info']} To add or modify investment accounts and categories, update the INVESTMENT_ACCOUNTS "
