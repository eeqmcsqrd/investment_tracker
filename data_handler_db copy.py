...
def save_data(df, filepath=None):
    """
    Save investment data to SQLite database.
    ...
    try:
        ...
        # Convert dates to strings in ISO format for SQLite storage
        df_to_save['date'] = df_to_save['date'].dt.strftime('%Y-%m-%d')
        
        # Clear existing data if replacing the entire dataset
...
def get_previous_values_db():
    """
    For each investment in the most recent date, find the previous value
    and calculate the delta, directly from the database.
    ...
    try:
        ...
        # Use a single query with window functions to get current values and previous values
        query = """
        ...
        PreviousValues AS (
            SELECT
                ...
                i.date,
                i.value,
                LAG(i.date) OVER (PARTITION BY i.investment ORDER BY i.date) AS prev_date,
                LAG(i.value) OVER (PARTITION BY i.investment ORDER BY i.date) AS prev_value
            FROM
                investments i
            WHERE
                i.investment IN (SELECT investment FROM LatestValues)
        )
        SELECT
            lv.investment,
            lv.currency,
            lv.value AS current_value,
            pv.prev_date AS previous_date,
            pv.prev_value AS previous_value
        FROM
            LatestValues lv
        LEFT JOIN
            PreviousValues pv ON lv.investment = pv.investment AND lv.date = pv.date
        """
        ...
        # Calculate deltas and USD values
        results = []
        for _, row in df.iterrows():
            investment = row['investment']
            currency = row['currency']
            current_value = row['current_value']
            current_value_usd = current_value * get_conversion_rate(currency)
            
            prev_date = row['previous_date']
            prev_value = row['previous_value']
            
            if pd.notna(prev_value):
                prev_value_usd = prev_value * get_conversion_rate(currency)
...
def calculate_comparison_metrics(portfolio_returns, benchmark_returns):
    ...
    if 'DailyReturn' in portfolio_filtered.columns and 'DailyReturn' in benchmark_filtered.columns:
        portfolio_volatility = portfolio_filtered['DailyReturn'].std() * (252 ** 0.5)  # Annualized
        benchmark_volatility = benchmark_filtered['DailyReturn'].std() * (252 ** 0.5)  # Annualized
        
        # Calculate beta (using covariance and variance)
        returns_cov = portfolio_filtered['DailyReturn'].cov(benchmark_filtered['DailyReturn'])
        benchmark_var = benchmark_filtered['DailyReturn'].var()
        
        beta = returns_cov / benchmark_var if benchmark_var != 0 else 1.0
        
        # Calculate tracking error (standard deviation of return differences)
        return_diff = portfolio_filtered['DailyReturn'] - benchmark_filtered['DailyReturn']
        tracking_error = return_diff.std() * (252 ** 0.5)  # Annualized
